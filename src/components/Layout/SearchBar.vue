...
<template>
  ...
  <div class="desktop-view">
    <div v-if="isSearch" @click="showSearchField"
      class="flex items-center shadow-md hover:cursor-pointer border border-gray-600 rounded p-1.5 flex-row">
    ...
    <div v-if="searchField" class="flex border border-gray-600 rounded p-1.5 items-center flex-row">
      <SearchIcon class="h-4" />
      <input class="ml-2 w-full" type="text" v-model="keyword" autofocus @focus="showSearchOptions"
        @blur="hideSearchOptions" />
      <XCircleIcon @click="hideSearchField" class="h-4 hover:cursor-pointer hover:text-lime-600" />
    </div>
    <div v-if="state.searchOptions"
      class="bg-white mt-1 bg-opacity-80 backdrop-blur-sm drop-shadow-lg rounded-b-lg w-80 h-60 z-40 overflow-y-auto text-sm absolute ">
      <div v-if="filteredShowNames.length > 0">
        <div class="grid grid-cols-1 divide-y" v-for="(show, index) in filteredShowNames" :key="index"
          @click="selectedShowName(show)">
          <div class="text-gray-900 p-2 border-b border-gray-400 hover:font-bold hover:text-black hover:cursor-pointer">
            {{ show.name }}</div>
        </div>
      </div>
  ...
  <div class="mobile-view">
    <div v-if="isSearch" @click="showSearchField"
      class="flex items-center mt-2 shadow-md hover:cursor-pointer border border-gray-600 rounded p-1.5 flex-row">
      <SearchIcon class="h-4" />
      <span class="text-sm ml-2 font-md">Search...</span>
    </div>
    ...
    <div v-if="state.searchOptions"
      class="bg-white mt-1 bg-opacity-80 backdrop-blur-sm drop-shadow-lg rounded-b-lg w-full h-60 z-40 overflow-y-auto text-sm absolute ">
      <div v-if="filteredShowNames.length > 0">
        <div class="grid grid-cols-1 divide-y" v-for="(show, index) in filteredShowNames" :key="index"
          @click="selectedShowName(show)">
          <div class="text-gray-900 p-2 border-b border-gray-400 hover:font-bold hover:text-black hover:cursor-pointer">
